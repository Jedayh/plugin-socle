...
class PaylineSDK
{
    ...
    protected function responseToArray($node, $parent = null)
    {
        $array = array();
        foreach ($node as $k => $v) {
            if ($this->isChildFromList($k, $parent)) {
                // current value is a list
                if ($v instanceof \Countable && count($v) == 1 && '0' != $k) {
                    // a list with 1 element. It's returned with a 0-index
    ...
    protected function webServiceRequest(array $array, array $WSRequest, $PaylineAPI, $Method)
    {
        ...
        try {
            ...
            if (true === $this->soapclient_options['trace']) {
    ...
    public function doAuthorization(array $array)
    {
        ...
        if (isset($array['payment']['mode'])) {
            if (("REC" == $array['payment']['mode']) || ("NX" == $array['payment']['mode'])) {
